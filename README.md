...
**老龙1井 D3+SVG 综合测井解释图**
- **模块化测井图引擎**：通用 D3.js + SVG 渲染引擎（`engine/`），配置驱动，支持 7 种列类型。`LaoLong1Chart.tsx` 为薄包装，通过 `configs/laolong1.ts` 驱动
- 14 列统一 SVG 渲染：地层系统、测井曲线（AC/GR、RT/RXO、SH/PERM/PHIE）、深度标尺、岩性（SVG 花纹）、岩性描述、沉积相、体系域、层序
- 每列 `<clipPath>` 防止内容溢出
- 文字按列宽自动换行（中文 0.85em 字符宽度估算）
- 学术风格表头：合并单元格、曲线图例、交替列底色、1m 间隔淡格网线 + 列分割线

**岩性 SVG 花纹（GB/T 勘探管理图件图册编制规范 附录M）**
| 岩性 | 花纹 | 底色 |
|------|------|------|
| 白云岩 | 菱形斜交线 | 浅蓝 |
| 砂岩 | 不规则圆点 | 浅黄 |
...
**地图井位显示**
- 57 口真实井位坐标（EPSG:2436 → WGS84 转换），地图上五星标记展示
- 红色五星 = 有数据的井（老龙1），灰色五星 = 无数据的井
- 点击红色五星 → 进入井详情页（数据表格 + 测井图两个选项卡）
- 路由：`/map`（地图）、`/well-detail/:wellName`（井详情）
...
| Phase 2 | ✅ 已完成 | 老龙1井 D3+SVG 综合测井解释图、交互编辑、悬停提示、地图五星标记、井详情页、岩性/沉积相纹理填充、模块化引擎重构、方法论文档 |
...
├── src-web/                # React 前端
│   └── src/
│       ├── pages/          # HomePage, MapHomePage, WellLogPage, LaoLong1Page, WellDetailPage
│       ├── components/
│       │   ├── well-log/   # LaoLong1Chart, LaoLong1Dashboard, WellLogDashboard
│       │   │   ├── engine/    # Generic chart engine (types, renderers, patterns, interaction, export, utils)
│       │   │   ├── configs/   # Per-well config (laolong1.ts)
│       │   │   └── patterns/  # SVG lithology pattern files (6 types)
│       │   ├── map/        # WellMap (MapLibre GL, star markers)
...
| 层级 | 框架 | 数量 | 覆盖范围 |
|------|------|------|----------|
| Python 后端 | pytest | 37 tests | API、数据模型、认证、数据生成 |
| React 前端 | vitest + @testing-library/react | 35 tests | 组件、页面、hooks、stores、i18n |
