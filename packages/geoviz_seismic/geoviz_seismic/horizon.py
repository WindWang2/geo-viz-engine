...
def extract_along_horizon(
    ...
    if window <= 0:
        ...
        with np.errstate(invalid="ignore"):
            # Rows where the horizon lies outside the volume or grid is NaN
            # leave the window all-NaN; nanmean would emit "Mean of empty
            # slice" for every such row on every call (#119).
