"""Map-edit geometry API: hit-test, vertex edit, snap, topology validation.

Promoted from ``paleo_workbench/mapping/map_edit_api.py`` (Phase-2
promote-down, map #244 / PR-A #256). Pure Python with optional ``map_edit_core``
(C++ / pybind11) acceleration for hot paths; falls back to pure Python when
the extension is absent.

C++ bridge refactor (vs the Workbench original): the Workbench version went
through ``paleo_workbench.native_backend`` (a dispatch registry that also
held lazy imports back into ``map_edit_api`` - a cycle). The promoted version
imports ``map_edit_core`` directly (T10: the extension source moves to
``geo-viz-engine/native/map_edit_core/`` and is built at the consumer side,
so it may be absent in the engine's own CI - the pure-Python fallbacks run).
The cycle is broken: no ``native_backend`` dependency, no lazy back-imports.
"""
...
# Optional C++ acceleration. The extension is built at the consumer side
# (T10: "source in submodule, build in consumer"); in the engine's own CI it
# is absent, so the pure-Python fallbacks are the exercised path there.
try:  # pragma: no cover - import guard
    import map_edit_core as _map_edit_core  # type: ignore[import-not-found]
    HAS_CPP = True
