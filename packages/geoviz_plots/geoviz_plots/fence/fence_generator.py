"""Cross-well fence mesh and inter-well seismic slice generator (headless numpy).

Promoted from `paleo_workbench/viz/geomodel/fence_generator.py` verbatim. This module
is the canonical implementation; the Workbench file becomes a facade re-export shim
(PR-B, mirroring the `segy_survey.py` precedent).

The sibling `paleo_workbench/viz/geomodel/well_seismic.py` previously held a near-
duplicate of this class whose `generate_fence_mesh` used a shared `max_depth` per
well pair and lacked the divide-by-zero guard; the per-well-depth variant here is
the correct one.
"""
...
def generate_fence_mesh(wells: list[dict], nz_samples: int = 20) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    """Module-level convenience wrapper around :meth:`CrossWellFenceGenerator.generate_fence_mesh`."""
    return CrossWellFenceGenerator.generate_fence_mesh(wells, nz_samples=nz_samples)
