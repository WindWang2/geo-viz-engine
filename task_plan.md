...
> **更新于 2026-07-04**：Phase 28（连井多曲线叠置 v0.14.0）、Phase 29-A/B/C（3D 高斯雕刻、矢量排版、等值线断层屏障 v0.17.0）、Phase 30（井震精细标定工作台 v0.18.0）、Phase 31（交叉图分析与岩性聚类工具箱 v0.19.0）、Phase 32（单井图像轨道与 LAS 测井解析 v0.20.0）已全面完成并通过 100% 单元/集成测试。

## Goal
GeoViz Engine 是一款基于 PySide6 的桌面地质数据可视化引擎。当前绝大部分功能已完成，900+ tests passed。**核心市场定位**：科研院所 + 中小油田 + 教学（差异化于 Petrel 的轻量、可二次开发、出版级出图）。

## Current State
- **Tests:** 全量单元与集成测试套件通过（含 44 项 Phase 28-32 专项与集成测试）。
- **Active phase:** Phase 32 单井图像轨道与 LAS 测井解析工具箱已合入完成 (v0.20.0)
- **Health rating:** A+（功能完整，连井/3D地震/矢量排版/井震标定/交叉图聚类/图像轨道已合入，全量测试与代码审查通过）
