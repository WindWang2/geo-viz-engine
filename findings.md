...
## Phase 11.8 — Level Lock & Overlaps (2026-06-01)

### 1. 全局层级锁定 (11.8-A 已修)
- **需求分析**：用户需要一个显式的“层级锁定”按钮/选择框。在选择并锁定某个级别（如 相/亚相/微相）后，无论如何拉动比例尺缩放，地图都将固定显示该选定级别，避免默认的“随比例尺自动切换图层”行为。
- **解决方案**：在 `src/pages/paleo_map/page.py` 的顶部工具栏中加入 `层级锁定` 组合框，并将其选项映射为 `set_locked_level(level)` 函数调用。`PaleoMapCanvas` 内部的 `_resolve_level_name()` 在 `self._locked_level` 非空时强制返回该锁定级别值，从而使整个画布（包括图层、标注、图例）均在所有显示比例尺下稳定展现锁定的级别。

### 2. 图例与首行元素重叠问题 (11.8-B 已修)
- **根因分析**：`LegendLayer.paint()` 中，绘制完 "图例" 标题后，facies swatches 色块的首行垂直 `y` 坐标起始点被设为 `y0 + PADDING + ROW_H + 4`，这导致首行图例元素过于靠近标题，出现严重的重叠违和感。
- **解决方案**：将首行色块的 `y` 坐标计算公式调整为 `y0 + PADDING + ROW_H + 12`，下移了 8 像素，使 "图例" 标题与第一行图例元素之间有充足的垂直留白间距，排版更加专业舒雅。

### 3. 比例尺滑动条文字重叠问题 (11.8-C 已修)
- **根因分析**：
  1. 滑动条上原本绘制了极具视觉干扰性的 transition labels（如 `相→亚相`、`亚相→微相` 等），且直接定位在阈值 tick 位置上，极易与滑块、其他文字水平重叠。
  2. `_make_ticks()` 在生成 tick labels（如 `1:100万`、`1:200万`、`1:1000万` 等）时没有做水平间距防撞保护，当大量细分刻度存在时，文字水平挤在一团形成大片重叠。
- **解决方案**：
  1. 彻底移除了滑动条上杂乱的 `相→亚相` 转换文本，改为在各自对应的彩色分段中央平铺“相”、“亚相”、“微相”的纯净文本。
